...
def get_balanced_dataset(path: str, one_hot_encode = False) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    df = pd.read_csv(path)
    df = df.drop(columns=["RefId", "PurchDate", "VehYear", "Trim", "Color",
             "WheelTypeID", "WheelType", "Nationality", "Size",
             "TopThreeAmericanName", "BYRNO", "VNST", "WarrantyCost"])
    
    # --------------- COMMENT ME TO FILL NA's ---------------
    df = df.dropna()
    # -------------------------------------------------------
    ...
    df_1_up = resample(df_1, replace=True, n_samples=df_0.shape[0] - df_1.shape[0], random_state=42)
